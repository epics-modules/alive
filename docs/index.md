...
The __alive__ record sends heartbeat information to a central server, allowing verification that an IOC is running. It also reports environment variables that may be requested remotely by the server.

The [alive record](aliveRecord.html) page describes the record's fields, operation, and network protocol in detail.

This record requires a separate server program outside of the scope of EPICS for it to be useful. The [alive server](aliveServer.html) page describes how to implement a server for receiving and processing the heartbeat messages, as well as making inquiries to the alive record about extra information. The author's version of such a server, __alived__, can be found [here](https://github.com/epics-alive-server/alived).
